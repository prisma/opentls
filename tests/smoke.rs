...
use cfg_if::cfg_if;
use env_logger;
use futures::{join, stream::StreamExt, AsyncWrite};
use opentls;
use opentls::{sync_io::TlsAcceptor, Identity};
use std::{io::Write, marker::Unpin, process::Command, ptr};
...
cfg_if! {
    if #[cfg(any(feature = "force-openssl",
        ...
        fn contexts() -> (opentls::async_io::TlsAcceptor, opentls::async_io::TlsConnector) {
            let keys = openssl_keys();

            let pkcs12 = t!(Identity::from_pkcs12(&keys.pkcs12_der, "foobar"));
            let srv = TlsAcceptor::builder(pkcs12);

            let cert = t!(opentls::Certificate::from_der(&keys.cert_der));
        ...
        const FRIENDLY_NAME: &'static str = "async-native-tls localhost testing cert";
...
The async-native-tls test suite is about to delete an old copy of one of its
...
description should mention "async-native-tls".
        ...
        fn install_certificate() -> io::Result<CertContext> {
            unsafe {
                ...
                let mut buffer = "async-native-tls test suite".encode_utf16()
                                                         .chain(Some(0))
                                                         .collect::<Vec<_>>();
                ...
                let name = "CN=localhost,O=async-native-tls,OU=async-native-tls,\
                            G=async-native_tls".encode_utf16()
                                          .chain(Some(0))
                                          .collect::<Vec<_>>();
...
The async-native-tls test suite is about to add a certificate to your set of root
and trusted certificates. This certificate should be for the domain "localhost"
with the description related to "async-native-tls". This certificate is only valid
for one day and will be automatically deleted if you re-run the async-native-tls
test suite later.
